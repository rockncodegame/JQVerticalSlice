...
public class PlayerAttack : MonoBehaviour {
	...
	void Update () {
		...
		//scale the attack up to signify a bigger attack
			...
			ChangeAttack (b2);
			b2.transform.localScale = new Vector3 (b2.transform.localScale.x * dir + .12f*dir, b2.transform.localScale.y + .12f, b2.transform.localScale.z);
		}
		//if the player continues the combo before the time is used, do the next move
		//if the first attack sprite is still there, destroy it
		//change the player state
		...
		//scale the attack up to signify a bigger attack
			...
			ChangeAttack (b3);
			b3.transform.localScale = new Vector3 (b3.transform.localScale.x * dir + .25f * dir, b3.transform.localScale.y + .25f, b3.transform.localScale.z);
		}
	}
